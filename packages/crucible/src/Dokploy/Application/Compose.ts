...
export type ApplicationCompose = Resource<
  "Crucible.Dokploy.Application.Compose",
      ...
      reconcile: Effect.fn(function* ({ id, news, output }) {
        if (!news || !isResolved(news)) {
          return yield* Effect.die(
            new Error("Crucible.Dokploy.Application.Compose: unresolved props at reconcile"),
          );
        }
        ...
        try {
          ...
          return yield* Effect.die(
            e instanceof Error
              ? e
              : new Error("Crucible.Dokploy.Application.Compose: invalid compose props"),
          );
        }
